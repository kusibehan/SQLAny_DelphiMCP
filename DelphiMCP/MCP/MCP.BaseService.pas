...
type
  ...
  protected
    ...
    function CreateInitializeResponse(const RequestID: string)
      : TJSONObject; virtual;
    function CreateJSONRPCResponse(const RequestID: string;
      inResult: TJSONValue): TJSONObject;
    function CreateJSONRPCErrorResponse(const RequestID: string;
      ErrorCode: Integer; const ErrorMsg: string; ErrorData: TJSONValue = nil)
      : TJSONObject;
    ...
    function HandleToolsCall(const RequestID: string; Params: TJSONObject)
      : TJSONObject; virtual;
    function HandleGetCapabilities(const RequestID: string)
      : TJSONObject; virtual;
    ...
    procedure ProcessJSONRPCRequest(Request: TJSONObject;
      out Response: TJSONObject); virtual;
    procedure ProcessRequest(const RequestStr: string;
      out ResponseStr: string); virtual;
  ...
  public
    constructor Create(const ServiceName, ServiceVersion,
...
constructor TMCPBaseService.Create(const ServiceName, ServiceVersion,
  ServiceInstructions: string);
begin
  inherited Create;
...
function TMCPBaseService.CreateInitializeResponse(const RequestID: string)
  : TJSONObject;
var
...
function TMCPBaseService.CreateJSONRPCResponse(const RequestID: string;
  inResult: TJSONValue): TJSONObject;
var
...
function TMCPBaseService.CreateJSONRPCErrorResponse(const RequestID: string;
  ErrorCode: Integer; const ErrorMsg: string; ErrorData: TJSONValue = nil)
  : TJSONObject;
var
  Error: TJSONObject;
...
begin
  ...
  begin
    ...
    if Params.TryGetValue<TJSONObject>('capabilities', FClientCapabilities) then
      FClientCapabilities := Params.GetValue<TJSONObject>('capabilities')
        .Clone as TJSONObject;

    // Store client info
    if Params.TryGetValue<TJSONObject>('clientInfo', FClientInfo) then
      FClientInfo := Params.GetValue<TJSONObject>('clientInfo')
        .Clone as TJSONObject;
...
function TMCPBaseService.HandleResourcesList(const RequestID: string)
  : TJSONObject;
var
  ResultObj: TJSONObject;
begin
...
function TMCPBaseService.HandlePromptsList(const RequestID: string)
  : TJSONObject;
var
  ResultObj: TJSONObject;
begin
...
function TMCPBaseService.HandleToolsCall(const RequestID: string;
...
begin
  ...
  begin
    // Missing tool name
    Result := CreateJSONRPCErrorResponse(RequestID, -32602,
  ...
  try
    ...
    begin
      ...
      var
      ErrorContent := TJSONObject.Create;
...
function TMCPBaseService.HandleGetCapabilities(const RequestID: string)
  : TJSONObject;
var
...
procedure TMCPBaseService.ProcessJSONRPCRequest(Request: TJSONObject;
...
procedure TMCPBaseService.ProcessRequest(const RequestStr: string;
...
begin
  ...
  try
    ...
    begin
      ...
      Response := CreateJSONRPCErrorResponse('', -32700,
        Format('Parse error: %s', [E.Message]));
      try
        ResponseStr := Response.ToString;
      finally
        Response.Free;
  ...
  begin
    try
      ...
      begin
        ...
        var
        ErrorResponse := CreateJSONRPCErrorResponse('', -32603,
